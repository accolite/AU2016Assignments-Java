...
chatboardCallBack = function(){
	if (xmlhttpc.readyState==4 && xmlhttpc.status==200){
		document.getElementById("chatbox").innerHTML = xmlhttpc.responseText;
    }
}
...
updateChatboard = function(){
	...
	xmlhttpc.open("GET","/ChatBox/chats",true);
...
updateActiveList = function(){
	...
	xmlhttpa.open("GET","/ChatBox/activelist",true);
	xmlhttpa.send();
...
sendMessage = function(){
	...
	if(msg.trim() !== ''){
		xmlhttp.open("GET","/ChatBox/post?msg="+msg,true);
