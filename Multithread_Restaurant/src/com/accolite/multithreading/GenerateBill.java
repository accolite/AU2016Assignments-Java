...
public class GenerateBill implements Runnable 
{
    ...
    public GenerateBill(Restaurant r) {
        restaurant = r;
	...
	public void run() {
		...
		try {
            while (!Thread.interrupted()) {
                ...
                restaurant.calcavgbill.lock.lock();
                try {
                    restaurant.bill = new Bill(count,amount);
                    restaurant.calcavgbill.condition.signalAll();
                } finally {
                    restaurant.calcavgbill.lock.unlock();
                }
