...
public class Main {
	...
	public List<Message> retrieve(){
		...
		try {
			messages = mapper.readValue(new File("file.json"), new TypeReference<List<Message>>(){});
		} catch (IOException e) {
			e.printStackTrace();
		}
		if(messages==null)
	...
	@POST
	@Consumes(MediaType.APPLICATION_JSON)
	public void test(String test) throws JsonParseException, JsonMappingException, IOException{
		ObjectMapper mapper = new ObjectMapper();
		Message message = mapper.readValue(test, Message.class);
		System.out.println("here "+test);
		System.out.println(message.getCreator());
	...
	public Response addMessage(String messageS) throws JsonParseException, JsonMappingException, IOException{
		...
		comments.add(new Comment("Ra","R"));
		List<Like> likes = new ArrayList<>();
		likes.add(new Like("RaL"));
