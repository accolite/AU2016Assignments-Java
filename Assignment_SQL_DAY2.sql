...
ProductDescriptionID int primary key identity(1,1) not null,
...
FROM ProductModel pm
	INNER JOIN Product p ON pm.ProductModelId = p.ProductModelId
	...
	INNER JOIN Customer c ON c.CustomerID = soh.CustomerID
	where pm.Name='Racing Socks'
--Query 7
SELECT Distinct COUNT(p1.ProductId)
FROM Product p1
 INNER JOIN ProductCategory pc1 ON p1.ProductCategoryId=pc1.ProductCategoryID
 INNER JOIN SalesOrderDetail sod1 ON sod1.ProductId=p1.ProductId
 INNER JOIN SalesOrderHeader soh1 ON soh1.SalesOrderID = sod1.SalesOrderId
 INNER JOIN Address a ON soh1.ShipToAddressID = a.AddressID
 where pc1.Name='Cranksets' and a.City='London'

 --Query 8
 
 SELECT Sum(soh1.SubTotal),a.CountryRegion
FROM SalesOrderHeader soh1
 INNER JOIN CustomerAddress ca ON ca.CustomerID = soh1.CustomerID
