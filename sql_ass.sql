...
Customer c1 where CustomerID=(select CustomerID from SalesOrderHeader SOH join SalesOrderDetail SOD ON SOH.SalesOrderID=SOD.SalesOrderID GROUP BY(CustomerID) HAVING sum(OrderQty*(UnitPrice-UnitPriceDiscount)+SOH.Freight+SOH.SubTotal+SOH.TaxAmt)> $1000)
Go
